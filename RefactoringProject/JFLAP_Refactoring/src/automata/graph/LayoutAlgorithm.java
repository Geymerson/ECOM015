...
public abstract class LayoutAlgorithm {
	...
	//Modified by Geymerson
	public static void polarToCartesian(AbstractGraph graph, ArrayList vertices) {
		Point2D polar, cartesian;
		for (int i=0; i<vertices.size(); i++) {
			polar = graph.pointForVertexA(vertices.get(i));
			cartesian = new Point2D.Double(Math.cos(polar.getY()) * polar.getX(),
							Math.sin(polar.getY()) * polar.getX());
			graph.moveVertexA(vertices.get(i), cartesian);
		}
	}
}
