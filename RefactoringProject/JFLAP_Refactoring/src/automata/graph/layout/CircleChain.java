...
public class CircleChain extends VertexChain {
	...
	public void layout(double r, double midTheta, double span)
	{
		...
		if (size() == 1) {
			if (r==0)
				graph.moveVertexA(vertices.get(0), new Point2D.Double(0, 0));
			else
				graph.moveVertexA(vertices.get(0), new Point2D.Double(r+diagonalLength, midTheta));
			return;
		}
		...
		for (int i=0; i<size(); i++)
			graph.moveVertexA(get(i), new Point2D.Double(radius, startTheta + thetaDivision * i));
	}
}
