...
import automata.graph.AbstractGraph;
import automata.graph.Graph;
import automata.graph.MyGraph;
...
public class VertexChain{
	...
	public int getDegreeInChain(Object vertex) 
	{
		...
		for (int i=0; i<vertices.size(); i++)
			if (graph.hasEdgeA(vertex, vertices.get(i)) && !vertices.get(i).equals(vertex))
				count++;
		return count;
	}
		 ...
		 for (int i=0; i<size(); i++)
			 if (graph.hasEdgeA(vertex, get(i))) {
				 ...
				 for (int j=i+2; j<size(); j++)
					 if (graph.hasEdgeA(vertex, get(j)) && getDegreeInChain(get(j)) <= 2) { 						 					
						 if (j<size()-1 && graph.hasEdgeA(get(j), get(j+1)))
						 	orientSubChain(destIndex, j, j, size()-1, (destIndex==i+1));
						 else {
						  	subChainBound = j;
						  	while (subChainBound>i+2 && graph.hasEdgeA(get(subChainBound-1), 
								get(subChainBound)))
							 	subChainBound--;
